...
2. **라운드 코너는 자동 처리하지 않는다**. UnityToFigma 가 SDF 로 처리한 결과는 그대로 두고, 그 외 모든 라운드/원형/알약 형태는 사용자에게 수동 작업 항목으로 보고만 한다. 자세한 규칙: `references/round-corner-policy.md`. **이 규칙을 깨고 sprite/mask 로 흉내내려 하지 말 것.**
...
| 명령 | 있으면 | 없으면 우회 |
|------|--------|-------------|
| `package add` | 그대로 사용 | manifest.json 직접 편집 + `editor refresh` |
| `menu execute` | 그대로 사용 | (UnityToFigma 자동 임포트 사용 불가 → Path B 폴백 권장) |
...
4. TMP Essentials 확인. 없으면 사용자에게 임포트 요청 (`Window > TextMeshPro > Import TMP Essential Resources`).
5. Editor 가 Play Mode 가 아닌지 확인 (`editor stop` 으로 정지 후 진행).
...
Console 한 줄 요약을 파싱한다:

```bash
unity-cli console get count=200 \
  | grep -E '^UnityToFigma import: created=[0-9]+, updated=[0-9]+, skipped=[0-9]+, failed=[0-9]+, orphaned=[0-9]+, manifestRemoved=[0-9]+'
```
...
- `screens` : `Assets/Figma/Screens/*.prefab` 목록
- `components` : `Assets/Figma/Components/*.prefab`
- `textures`, `serverImages`, `fonts`
- `roundedSkipped` : 라운드 처리 후보 (사용자 보고용. 보정 시도 금지)
...
| **라운드 코너 차이** | **수정하지 않음.** `roundedSkipped` 항목으로 누적해 Phase 5 에서 사용자에게 보고. (`references/round-corner-policy.md`) |
...
cp ~/.claude/skills/ugui-from-screenshot/assets/FixTMPSettings.cs Assets/_Project/Scripts/Editor/
unity-cli editor refresh && unity-cli editor compile
...
### 생성 산출물
Assets/Figma/Screens/    : N 프리팹
Assets/Figma/Components/ : N 프리팹
Assets/Figma/Pages/      : N 프리팹
Assets/Figma/Textures/   : N 이미지
Assets/Figma/ServerRenderedImages/ : N 벡터 PNG
Assets/Figma/Fonts/      : N 폰트 (Google Fonts 다운로드 포함)
...
{roundedSkipped 목록을 그대로 출력. 처리 방법은 references/round-corner-policy.md 의 안내 문구 참조}

### 사용자 후속 작업 안내
- 라운드 코너 직접 처리: 위 항목들에 대해 9-slice sprite / SDF cornerRadius / 표준 컴포넌트 적용
